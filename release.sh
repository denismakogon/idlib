...
version_file="${artifact}/version.py"
...
tag="$new_version"
git add -u
git commit -m "Python Object ID: $new_version release [skip ci]"
git tag -f -a $tag -m "version $new_version"
git push
git push origin $tag

PBR_VERSION=${new_version} python setup.py sdist bdist_wheel
twine upload -u ${PYPI_USER} -p ${PYPI_PSWD} dist/${artifact}-${new_version}*
